...
"""Dynamic CMU ARCTIC example (fixed sources + moving mic array).

This script:
1) Loads random CMU ARCTIC utterances for multiple speakers.
2) Samples fixed source positions and generates a moving mic array trajectory.
...
- dynamic_mic.wav
- dynamic_mic_ref01.wav, dynamic_mic_ref02.wav, ... (per-source convolved references)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Dynamic RIR: fixed sources, moving mic array"
    )
    parser.add_argument(
        "--dataset-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Number of microphones in the moving array.",
    )
    parser.add_argument(
        "--duration",
        type=float,
    ...
    if args.num_mics == 2:
        mic_traj = torch.stack(
            [
                arrays.binaural_array(center, offset=MIC_SPACING)
                for center in mic_center_traj
            ],
            dim=0,
        )
    else:
        mic_traj = torch.stack(
            [
                arrays.linear_array(
                    center, num=args.num_mics, spacing=MIC_SPACING, axis=0
                )
                for center in mic_center_traj
            ],
            dim=0,
    ...
    for src_idx in range(args.num_sources):
        ...
        ref_name = f"dynamic_mic_ref{src_idx + 1:02d}.wav"
        save_scene_audio(
            out_dir=args.out_dir,
            audio=ref,
            fs=fs,
    ...
    save_scene_audio(
        ...
        audio=y_dynamic,
        fs=fs,
        audio_name="dynamic_mic.wav",
        logger=logger,
    )
    metadata = save_scene_metadata(
        out_dir=args.out_dir,
        metadata_name="dynamic_mic_metadata.json",
        room=room,
        sources=sources,
        mics=mics,
        rirs=rirs,
        ...
        signal_len=signals.shape[1],
        source_info=info,
        extra={"mode": "dynamic_mic", "reference_audio": reference_audio},
        logger=logger,
    )
